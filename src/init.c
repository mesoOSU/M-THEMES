...
static void InitMicroStruct(char *s)
{
	...
	local_loop{ 
		// access heap arrays with pointer arithmetic
		ph = *(euler_data + px*CellDim[1]*CellDim[2]*3 + py*CellDim[2]*3 + pz*3); // phi
		th = *(euler_data + px*CellDim[1]*CellDim[2]*3 + py*CellDim[2]*3 + pz*3 + 1); // theta 
		om = *(euler_data + px*CellDim[1]*CellDim[2]*3 + py*CellDim[2]*3 + pz*3 + 2); // omega

		jgr = *(grain_data + px*CellDim[1]*CellDim[2] + py*CellDim[2] + pz); // grain ID
		jph = *(phase_data + px*CellDim[1]*CellDim[2] + py*CellDim[2] + pz); // phase ID
...
#ifdef DD_CU
		ElastConst_PhaseI[0] = 241.1674*1000;//(1.818-0.0004045*T__K)*100*1000;	// MPa
		ElastConst_PhaseI[1] = 89.1989*1000;//(1.285-0.0002*T__K)*100*1000;	// MPa
		ElastConst_PhaseI[2] = 151.9685*1000; //(0.8396-0.0002711*T__K)*100*1000;	// MPa
#endif
...
#endif
#ifdef DD_CU
		ElastConst_PhaseII[0] = 0.0; //(1.818-0.0004045*T__K)*100*1000;	// MPa
		ElastConst_PhaseII[1] = 0.0; //(1.285-0.0002*T__K)*100*1000;	// MPa
		ElastConst_PhaseII[2] = 0.0; //(0.8396-0.0002711*T__K)*100*1000;	// MPa
#endif
...
//exit(0);
 ...
 //   grex_new[pIDX] = 0;
	...
	for(jph=0;jph<NPHMAX;jph++){
		...
		InteractionStrengthMatrix("FCC",Selfinter[jph],Coplanar[jph],
