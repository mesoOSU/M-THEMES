...
#else
		/* Consider both forward and backward jumps */
		tau_cut = (kT/MY_J_IN_EV)*sqrt(rho_F[idx][i])/c_2/c_3/bb_B[jph-1][i]/bb_B[jph-1][i]*0.1;	// MPa
		//if(grex_new[idx]){
      rho_m[idx][i] = 2.0*kT/c_1/c_2/c_3/shear_G/(pow(bb_B[jph-1][i],3.0))*sqrt(rho_P[idx][i]*rho_F[idx][i])*160.2177;
		 gam_o = 1E-3*(rho_m[idx][i])*TRIAL_FREQ_DRX*c_2*bb_B[jph-1][i]*exp(-1.0*q_slip)/sqrt(rho_F[idx][i]);
			...
			for(i=0;i<nSYS[jph-1];i++){
				...
				if(istep<=2000000) {
				  //  rho_m[pIDX][i] += (fabs(gamdot[pIDX][i])*(c_4*sqrt(rho_F[pIDX][i])) - fabs(gamdot[pIDX][i])*(c_6*d_dipole*rho_m[pIDX][i]))*TimeStep;
				    rho_dipole[pIDX][i] += fabs(gamdot[pIDX][i])*(c_6*(d_dipole)*rho_m[pIDX][i])*TimeStep;
				    rho_forest[pIDX][i] += fabs(gamdot[pIDX][i])*(c_4*sqrt(rho_F[pIDX][i]))*TimeStep;
				dot_SSD = fabs(gamdot[pIDX][i])*(c_4*sqrt(rho_F[pIDX][i]) +
					c_6*(d_dipole)*rho_m[pIDX][i] - c_5*rho_s[pIDX][i]) - c_7*1.6*1E12*1E-9*rho_s[pIDX][i]*rho_s[pIDX][i] - 4*1E12*(d_dipole)*(D_0/bb_B[jph-1][i])*pow(rho_s[pIDX][i],2.0)*exp((fabs(hydro[i])*bb_B[jph-1][i]*bb_B[jph-1][i]*bb_B[jph-1][i]*100/(1.38*T__K)) - 1.0)*exp(-1.0*q_bulk);
